...
class Infortis_Ultimo_Helper_Labels extends Mage_Core_Helper_Abstract
{
	 ...
	 * @return string
	 */
	public function getLabels_ALT($product, $size = '')
	{
		$html = '';
		...
		if ($isNew == true)
		{
			$html .= '<span class="sticker-wrapper top-left"><span class="sticker new ' . $size . '">' . $this->__('New') . '</span></span>';
		}
		
		if ($isSale == true)
		{
			$html .= '<span class="sticker-wrapper top-right"><span class="sticker sale ' . $size . '">' . $this->__('Sale') . '</span></span>';
		}
