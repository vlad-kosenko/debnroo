...
class MDN_Purchase_Model_Order extends Mage_Core_Model_Abstract {
    ...
    //DEV CUSTOM - TICKET 11501 - BOOTMYSHOP - 2014 03 12 - BEGIN
    /**
     * generate the next PO number
     * ex from customer : 2014000627, 2014000628, etc.
     * PATTERN is YEAR000000
     * +1 ahve to be done each time
     * if we reach next year go to ex : 2015000001
     */
    public function GenerateOrderNumber() {
       
        $currentYear = date('Y');

        //a default value in case of problem and to go to the next year
        $newNumber = $currentYear . '000000';

        $tablePrefix = Mage::getConfig()->getTablePrefix();
        $conn = mage::getResourceModel('sales/order_item_collection')->getConnection();

        //get last one
        $sql = 'SELECT po_order_id ';
        $sql .= 'FROM '.$tablePrefix.'purchase_order ';
        $sql .= 'ORDER BY po_num DESC LIMIT 1';
        $lastPoNum = $conn->fetchOne($sql);

        if(strlen($lastPoNum)>4){
            $yearOfLastPoNum = substr($lastPoNum, 0, 4);
            if($yearOfLastPoNum < $currentYear){
                $lastPoNum = $newNumber;
            }
        }

        //find the next one (max : 3000 PO by day !)
        for ($i = 1; $i < 3000; $i++) {

            //define the new number
            $newNumber = $lastPoNum + $i;

            $sql = 'SELECT count(po_order_id) ';
            $sql .= 'FROM '.$tablePrefix.'purchase_order ';
            $sql .= 'WHERE po_order_id="'.$newNumber.'"';
    ...
    //DEV CUSTOM - TICKET 11501 - BOOTMYSHOP - 2014 03 12 - END
    /**
     *
     *
     */
    public function GenerateOrderNumberOLD() {
        //prefix
        $retour = date('Ymd') . 'PO';
