       ...
       foreach ($productListToFix as $diffItem) {
            try {
               //$helper->RefreshForOneProduct($diffItem['pa_product_id']);   
                $product = mage::getModel('catalog/product')->load($diffItem['pa_product_id']);
                mage::helper('AdvancedStock/Product_Base')->updateStocks($product);
                //mage::helper('BackgroundTask')->AddTask('Update product availability status for product #' . $diffItem['pa_product_id'], 'SalesOrderPlanning/ProductAvailabilityStatus', 'RefreshForOneProduct', $diffItem['pa_product_id']);
